...
const SpecialOfferCardRectangle: React.FC<SpecialOfferCardRectangleProps> = ({
  bouquet,
  onViewBouquet,
}) => {
  ...
  return (
    <div
      className="col-span-2 flex h-full flex-col rounded-xl bg-bg-card p-2 shadow md:col-span-1 md:justify-self-center md:p-4"
      onClick={() => handleViewBouquet(bouquet)}
    >
      <div className="relative w-full overflow-hidden rounded-lg pt-[50%] md:flex-1 md:pt-0">
        <img
          src={bouquet.images[0]}
          alt={bouquet.name}
          className="absolute left-0 top-0 h-full w-full object-cover md:relative md:h-full md:w-full"
        />
      </div>
